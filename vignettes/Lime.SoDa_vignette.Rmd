...
```
Precision Liming Soil Datasets (Lime.SoDa) is a collection of datasets from a field- and farm-scale soil mapping context. Datasets are ready “ready-to-go” for modelling, meaning they contain target soil properties and features in a tabular format. Target soil properties for all datasets are; soil organic matter (SOM) or -carbon (SOC), pH and clay. Features for modelling are dataset-specific. The goal of `Lime.SoDa` is to make benchmarking and comparison of different modelling approaches in Digital Soil Mapping and Pedometrics more trustworthy by providing an open collection of datasets. This vignette provides an overview of the `Lime.SoDa` package and how to use it for modelling purposes.
...
`Lime.SoDa` contains 31 entries. This inclues 30 datasets and an overview table.

```{r}
data(package = "Lime.SoDa")$results[,3] # All entries in Lime.SoDa
```
`Overview_Datasets` provides an overview of the 30 datasets, including the dataset name, number of samples, number of features and further specifics:
