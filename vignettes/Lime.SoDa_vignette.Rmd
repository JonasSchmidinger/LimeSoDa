...
library(Lime.SoDa)
library(sf) # For visualizing spatial data later on
```

### Lime.SoDa package structure
`Lime.SoDa` consists of 29 tabular datasets that are ready-to-go for modelling purposes:

```{r}
data(package = "Lime.SoDa")$results[,3] # List datasets from Lime.SoDa
```

The dataset codes are named according to the geographic region and sample size. For example, `BB.250` refers to a dataset from Brandenburg (BB), Germany, which includes 250 reference soil samples. Additional information about the methodology, sampling procedures, feature codes, and more can be found in the metadata of each dataset:

```{r}
?BB.250 #Access metadata
```

Datasets are stored as a list. This list contains the tabular dataset (`$Dataset`), pre-defined folds (`$Folds`) for a 10-fold cross-validation  and, if available, the coordinates (`$Coordinates`):
...
Features are specific to each dataset and may consist of lab-based spectroscopy, in-situ proximal soil sensors or remote sensing derived environmental data, depending on the context of the dataset:
...
Spatial coordinates are available for many datasets. For some datasets they could not be provided (e.g., `SSP.58`) or only dummy coordinates are available (e.g., `UL.120`), for reasons specified in the metadata. For example, in `BB.250` coordinates are included and it can be accessed through `$Coordinates`. The EPSG code is provided in the column name and metadata. Conversely, in `SSP.58` coordinates were excluded, so that `$Coordinates` returns an `NA` value. In the case of `UL.120`, `$Coordinates` returns dummy coordinates which is highlighted by the column names:
...
The coordinates are arranged in the same order as the tabular dataset. This allows them to be easily merged with the dataset using a simple `cbind()`:
